...
__all__ = ["AtomisticModel", "PairwiseModel", "PairwiseModel_Mod"]
...
class PairwiseModel_Mod(nn.Module):
    """
    Join a representation model with output modules.
    ...
    def __init__(self, representation, output_modules):
        super(PairwiseModel_Mod, self).__init__()
        self.representation = representation
        if type(output_modules) not in [list, nn.ModuleList]:
            output_modules = [output_modules]
        if type(output_modules) == list:
    ...
    def forward(self, inputs):
        ...
        ZB = inputs["ZB"]
        ZA = Z.unsqueeze(-1)
        ZA = torch.repeat_interleave(ZA, repeats=ZB.shape[2], dim=2)
        ZA = torch.reshape(ZA, (ZA.shape[0], ZA.shape[1]*ZA.shape[2]))
        ZB = torch.reshape(ZB, (ZB.shape[0], ZB.shape[1]*ZB.shape[2]))

        radial_sf, ap_sf, pair_dists, fd = self.representation(inputs)
        radial_sf = radial_sf.unsqueeze(2)
        radial_sf = torch.repeat_interleave(radial_sf, repeats=ap_sf.shape[2], dim=2)
        radial_sf = torch.reshape(radial_sf, (radial_sf.shape[0], radial_sf.shape[1]*radial_sf.shape[2], radial_sf.shape[3]))
        ap_sf = torch.reshape(ap_sf, (ap_sf.shape[0], ap_sf.shape[1]*ap_sf.shape[2], ap_sf.shape[3]))

        inputs["representation"] = (ZA, ZB, pair_dists, radial_sf, ap_sf, fd)
        outs = {}
        for output_model in self.output_modules:
            outs.update(output_model(inputs))
        return outs
